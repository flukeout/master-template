...
function newContent(content){
  $(".page").html(content);
  var pads = $(".page .etherpad");
  pads.each(function(i,el){
    var url = $(el).text();

    var iframe = $("<iframe class='etherpad-embed' />");
    iframe.attr("src",url);
...
tinymce.init({
  ...
  templates: [
    {title: 'Box List', description: 'Embedded Etherpad', url: 'templates/etherpad.html'}
  ]
});
